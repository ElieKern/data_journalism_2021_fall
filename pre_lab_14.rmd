...
**Answer**
The dataframe it outputs has 152584 rows and 12 columns, and it includes a variety of data about COVID related tweets.
```{r}
...
**Answer**
The 'unique' filters the list of words so that it only shows each type of quote once.  
```{r}
...
**Answer**
The unnest_tokens function takes each unique word and puts it in its own row. 
```{r}
...
**Answer** I do not find the resulting graphic to be particularly interesting.  The top words are all pretty standard words for tweets, such as "the", "to", "and" and "of."
...
**Answer**
I find it the results to now be much more interesting, as it has removed all stop words and now only includes covid-related words.
```{r}
unique_words %>%
...
**Answer**
There seems to be much more usage of "vaccine" and similar words in 2021. "Testing" is also no longer a top word.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
There are some interesting differences.  Perhaps most noticeably, "health care" and "public health" are the second and third highest terms on the entire list.
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Terms such as "public health" and "health care" have fallen since the previous year.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer** We now see seperate dataframes for the tweets of Democrats and Republicans.  I am not surprised to see that Democrats tweet more about health care and social distancing.
...
**Task** Run the following code and describe the results.
**Answer**
The difference in results make a lot of sense to me.  2020 tweeted much more about President Trump and nursing homes, while 2021 tweeted more about President Biden and the covid-19 vaccine.
```{r}
covid_tweets %>%
...
**Answer**
I am not sure if Trump is typically tweeted about in a positive way.  While the word Trump may have a positive definition, the tweets are most likely about the man, not the word.  
```{r}
