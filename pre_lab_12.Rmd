...
**Answer**
There are 14 columns, all related to county vaccination information by date.
```{r}
...
**Answer**
There is one county that just says "NA" instead of a name.
...
**Answer**
I think this data represents the complete statistics for Maryland jurisdictions, but it removes the NA data.
...
**Answer**
The most recent dates are all ranked lower numbers. This is probably due to the fact that more people are vaccinated overall.
```{r}
...
**Answer**
The trend in the data seems to decline over time, as more people are vaccinated overall.
```{r fig.width=9}
ggplot() +
...
**Answer**
According to sub: "The number of people in Prince George's County fully vaccinated on November 14 decreased by -74.25 percent compared to the week before"
```{r}
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer**
The text of sub is now below the title.
```{r fig.width=9}
ggplot() +
  geom_bar(data=ranked, aes(x=vaccination_date, weight=fully_vaccinated)) +
  geom_bar(data=latest_date, aes(x=vaccination_date, weight=fully_vaccinated), fill="red") +
...
**Answer**
The resulting chart shows the growth in cases over time by county, with each county having its own color code.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
The resulting chart includes an overall logging of the jump in rates of new cases. 
```{r}
...
**Answer**
There are  3,233 rows in the counties dataframe.
```{r}
...
**Answer**
There are 25 rows in the mdcounties dataframe.
```{r}
...
**Answer**
The map is showing how many total covid cases each county has of 11/15/2021, and it is color coded by the number of cases.
```{r}
ggplot() +
