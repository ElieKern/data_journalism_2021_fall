...
**Answer**
I see a list of different files that all begin with "hospitals."
```{r, echo=FALSE}
...
**Answer**
The columns consist of a range of information about hospitals, such as name, address, city, and many more details.  
```{r}
...
**Answer**
The output is just like the hospitals dataframe, but filtered to only show Maryland.
```{r}
...
Just hospitals in Maryland.
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**
It lists the hospitals in descending order of their "count." However, I am unsure what the count is referring to.  
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**  MD hospitals have been filtered to only show General Acute Care types.  
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This appears to be a scatter plot, and it depicts the general areas that these hospitals are located.
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The package and libraries have been installed.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**  We now have a large dataframe that details geometric information about each county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now only see counties in Maryland, by using the FP number as a filter.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now see a map of the counties within the state of Maryland.
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We can now see exactly where the hospitals are located within each county.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The points are now color coded by how many beds there are, which helps us determine how big each hospital is.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now have a dataframe of Maryland hospitals, filtered for the state's 50 General Acute Care hospitals.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**  We see a descending list of total beds by county.  
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**  We see a descending list of total population by county.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now have coordinates of some of our counties.  
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have a new data frame that has the beds by county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now have an updated md_county_population, with NAME replaced by COUNTY, removing ", "Maryland" and County", all extra white space removed, and the column that contains the population information has been renamed from "estimate" to "population". 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now have a new dataframe of md_beds_per_100k, which combines md_county_population and md_beds_by_county.  
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have now removed NA from our dataset, and replaced it with 0.  
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We have now added the column we have been looking for: beds_per_100k.  We have also arranged it to show in descending order.   
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**  We have a chloropleth map of beds_per_100k, with the lighter shades of blue representing more hospital beds per 100k.  
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We  have the same map, but with a color scale that makes it easier to see the differences.  
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
The color scale has gotten even clearer.  It is interesting to see that the southern portion of the state has the lowest beds_per_100k.  
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
