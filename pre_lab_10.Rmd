...
https://datawrapper.dwcdn.net/McaTv/2/
...
**Answer** It is grouping the data by county and is listing how many approved applications each county has.
...
**Answer** It is filtering the data so that it only shows counties with more than 6 approved applications.
...
The new 5 digit 'fips_code' column allows us to see the state code combined with the county code.
...
In 'md_counties_with_fips," I see a combination of the 'md_counties' and 'md_fips.' There are a few places that do not have fips codes here, and the state and county is written twice.
```{r}
...
The other map was extremely zoomed out and made Maryland hard to find.  This map is just Maryland.
...
**Answer** Describe the changes from the previous map
Now, when I hover over a certain county, I can see both the county name and the amount of approved PPP loans.
...
https://datawrapper.dwcdn.net/iZMSe/1/
