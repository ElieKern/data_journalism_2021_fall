...
It might be useful to have a lookup table of those top-level, two-digit NAICS codes (also called sector codes) for our analysis, to help us answer questions about what specific top-level industries got loans.  
...
**Answer**
I see an Industry Classification Overview, which helps with the understanding of Current Employment Statistics.  The North American Industry Classification System is also known as NAICS, which is a term we have grown to be quite familiar with this semester.
### Task 3: Load image
...
**Answer**
I see a code describing every HTML detail of the of the website's page.  This is fascinating to me that we have access to such information, as it gives me the resources to learn how to emulate any web page.  
...
**Answer**
The output has a head and a body, with a body class titled "layout-fixed."
```{r}
# read in the html
...
**Answer**
I see a column of names within html that include head, body, and xml attributes.  The next column is the "type", and the third column is the "value".
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
I see lots of elements making a very detailed and complete html code.  
...
**Answer**
It has 6 boxes and an R console box, and each box has different columns within it.
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
I am seeing  a nested list that contains six different data frames, and they are numbered 1 to 6. The first 1 has 4 rows and 3 columns, the second has 21 rows and 2 columns, etc.
...
**Answer**
I see 3 columns by 4 rows.  The columns are labeled Conversion,Reference Month Implemented, and Date Released.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
I see 2 columns by 21 rows. The columns are labeled sector and description.
### Task 16: Load image
...
**Answer**
I see a 21 x 2 tibble that lists a range of numbers under the sector column and a range of types of work under the description column.
```{r}
...
**Answer**
It seems to be almost the same, but one row less.
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
 I see a web page that has detailed employment information for NAICS sectors 22: Mining, Quarrying, and Oil and Gas Extraction.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
Below the codeblock I see a line of {html_document}, then <html lang="en">, than a header line, and then a body line.
```{r}
...
**Answer**
Below the codeblock, I see a line of {html_node}, a line of <table class="regular" id="iag22emp1">, followed by a caption line, a thead line, a tbody line, and a tfoot line.
```{r}
...
**Answer**
I see a 6x6 tibble, that has columns saying data series, backdata, jun.2021, jul.2021, aug.2021, and sep.2021.
```{r}
...
**Answer**
The columns are now just data_series and jun_2021.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see the NAICS information for Mining, Quarrying, and Oil and Gas Extraction.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see the NAICS information for Construction.
...
**Answer** I see a clear and manageable list of 10 industry sectors.
```{r}
...
**Answer** In the console, I see naics_industry[[2]][["Sector"]].  However, I see nothing below the codeblock.
```{r}
...
**Answer** I see a clear and organized list of sectors.
```{r}
...
**Answer**
Below the codeblock, I see an error saying "Error in sector_list : object 'sector_list' not found".
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** Once again, below the codeblock I see a clear and organized list of sectors.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** Below the codeblock, I see an error saying "Error in print(sector_name) : object 'sector_name' not found."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** There is a list counting from 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
There is 20 x 2 tibble that has columns labeled sector and description.
```{r}
...
**Answer** There is just the number 20.
...
**Answer**
There is a list counting from 1-20.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
There is 1x2 tibble that has rows labeled data_series and jun_2021.
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** Below the codeblock there is a 20x2 tibble with columns labeled sector and description.
```{r}
# Define url of page we want to scrape
...
**Answer**
I see a 20x3 tibble with columns labeled sector, description, and sector_url.
```{r}
...
**Answer**
I see a 20x4 tibble with columns labeled sector, description, sector_url, and sector_xpath_id.
```{r}
...
**Answer**
I see the same tibble as before with one less row.
```{r}
...
**Answer**
I see 19 seperate tibbles, each one being 1x4 and including columns labeled sector, description, sector_url, and
sector_xpath_id.
```{r}
...
**Answer** Below the codeblock are 19 rows that ionclude 19 seperate URLS links.
```{r}
...
**Answer**
Below the codeblock, there are 20 boxes.  The tibbles have different dimensions, such as 2x6, 4x6, 6x6, 9x6, and 10x6.
```{r}
...
**Answer**
I see 19 boxes, with all tibbles being 1 x 6.  The columns include data_series,backdata, jun_2021, jul_2021, aug_2021, and sep_2021. 
```{r}
...
**Answer** 
There are 19 boxes, each with a 1x10 tibble.  The columns say data_series,backdata, jun_2021, jul_2021, aug_2021, sep_2021, sector, description, 	sector_url, and sector_xpath_id.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
Below the codeblock, there are 19 boxes, each one with a 1x3 tibble, the columns include: sector, description, and jun_2021_employees.  
```{r}
...
**Answer**
There is a 19x3 tibble, that has columns labeled sector, description, and jun_2021_employees.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see the NAICS information for Agriculture, Forestry, Fishing and Hunting.
### Task 39: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Answer**
There is a 19x3 tibble, with columns labeled sector, description, and jun_2021_employees. 
```{r}
