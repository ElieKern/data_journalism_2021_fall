...
```{r}
maryland_ppp_naics <- maryland_ppp %>% left_join(naics_codes, by = "naics_code") %>%
...
```{r}
maryland_ppp_with_naics_and_demographics <- maryland_ppp_naics %>% left_join(maryland_zcta, by=c("zip5"="ZCTA5N")) 
```
...
Taxi services had the most with 7,363. The next most were beauty salons and restaurants.
```{r}
...
Restaurants had the most with 1,488 applications.  The next most were physicians and plumbing, heating, and air-conditioning contractors.
```{r}
maryland_ppp_with_naics_and_demographics %>%
...
**A3** 
The leader was 20850.  It had $474,525,058 in loans.
...
**A4** 
The zip code that has the largest sum was 20850 with $28,994,798 in loans. 23.5% of its population is Asian. 
```{r}
maryland_ppp_with_naics_and_demographics %>%
  filter(race == 'Asian') %>%
  group_by(zip5, PNHA) %>%
...
**A5** 
One possible reason is that many people of Asian descent may have chosen not to fill out the race column with the PPP loans.
...
**A6** This happens to zip code 20131.  This may be because zip code has no population, just businesses.   
...
**A7** The code below gives me an answer of 0 industries.
