...
**Answer**
I do not see any parsing errors.
```{r}
...
**Answer**:NA is 192959. Subway is 275.
...
**Answer**: NA is 135073, it is the same the number as forgiveness amount.
...
**Answer**:The count is 135,073. You can conclude from this that the data is consistent and
```{r}
ppp_maryland_loans %>%
...
**Answer** There do not seem to be any loans in October 2020.
```{r}
ppp_maryland_loans %>%
...
**Answer** The Max amount 10,000,000.  The smallest loan is for 6. Both numbers are interesting, because a loan of 10,000,000 dollars should be investigated to understand why such a big loan was necessary, and a loan of 6 dollars seems to small to be of any practical purpose.
```{r}
...
**Answer**  It seems as though there are two loans.  One to GETGFTD LLC, and one to LEGACY SPINE AND PAIN LLC.
```{r}
...
**Answer** The column headers is inconsistently styled, the amount column is stored as a character instead of a number, and there's a fully duplicated row.
```{r}
arnold_md_loans
```
...
**Answer** It says "Error: Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument Run `rlang::last_error()` to see where the error occurred."  I think it means that the amount is not in a number format.
...
**Answer** There are two rows of Stuart Jones LLC.
