...
WV_ppp_loans %>%
  ...
  geom_text(aes(x=date_approved, y=total_loans, label=total_loans),hjust=1.25) +
    scale_x_date(date_breaks = "1 day", date_labels = "%d-%Y") +
   theme(
    axis.text.x = element_text(angle = 45,  vjust=1)) +
labs(
  title="West Virgnia April PPP Loans",
    x = "date",
    y = "loans",
...
```
**A1.** 
The 28th of April saw the most loans.
...
**A2.** 
The top 10 states that got the most PPP loans per 100,000 people, respectively, are: South Dakota, North Dakota, Nebraska, Georgia, Iowa, Illinois, Louisiana, Florida, Mississippi, and Montana.
```{r}
state_ppp_all <- read_csv("data/state_ppp_all.csv")
  
acs5<- load_variables(2019, "acs5", cache = TRUE)

clean_state_ppp <- state_ppp_all %>% 
...
total_pop_state <- get_acs(geography = "state",
              variables = c(total = "B01001_001"),
              year = 2019)
new_data <-
clean_state_ppp %>%
  inner_join(total_pop_state, by=c("state" = "NAME")) %>% 
  mutate(loans_per_100000=total_ppp_loans/estimate*100000) %>% 
  filter(loans_per_100000 >= "4443") %>% 
  arrange(desc(loans_per_100000)) 
  
new_data %>% 
  ggplot() +
  geom_bar(aes(x=reorder(state,loans_per_100000), weight=loans_per_100000), fill="steelblue") +
labs(
  title="Loan Ratio by State",
    x = "State",
    y = "Loans Per 100,000",
...
**A3.**  
```{r}
filtered_WV_ppp_loans <- read_rds("data/lab_09.rds") %>% 
  filter("amount" < 25000)
