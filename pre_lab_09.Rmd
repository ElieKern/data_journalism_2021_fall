...
**Answer**
The first step us rename a dataset.  Within that dataset, we then look specifically at project county names.  We are then getting more specific, and looking at the number of total loans for each of those counties.  Next, we arrange them in a way that goes from highest to lowest.  Finally, we use the head() function to look at the first 10 parts of the data.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
I see a bar graph.  It has the different county names listed horizontally, and loan counts listed vertically.  
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
The counties are now listed in ascending order.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The coordinates have been flipped.  The county names are now listed vertically, and the count of loans is listed horizontally. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The counties have been reordered.  It starts with the highest county in loans, and it continues descending.  
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer** The fonts and colors have changed, and there is now a blue background.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
There is now a title and a source section.  
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**  I see a tibble with 235 rows and 2 columns.  The columns are labeled date_approved and total_loans.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**  I see a line graph, with many loans at the beginning of the program. There is another spike at the beginning of 2021, and moderate fluctuation until July 2021. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
There is now one tick and mark and label per month on the x axis.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The months are now clearly labeled. They are spread apart, and it is much more readable.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
