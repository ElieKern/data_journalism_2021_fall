...
**Answer**The dataframe has five columns by 51 rows, describing the voting patterns and PPP related information per state.
```{r}
...
**Answer** I see a plot that show a slight trend from the bottom left to the top right.  
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** I see a plot without any clear slope or trend.  
```{r}
...
**Answer** The table output shows the relationship between a state's pct_trump, businesses_receiving_ppe_pct, and ppe_amount_per employee.  There is a positive, albeit small, relationship.  
```{r}
reveal_data %>%
...
**Answer** I see several different forms of statisticts, such as the 95 percent confidence interval and the sample estimates correlation (or R value), being interpreted.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** I see the same tests being run, with a lower R value and a higher P value.
```{r}
...
**Answer** I see the mean ppp amount per employee and the mena businesses receiving ppp pct for both Clinton and Trump in 2016.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** I see the mean for Trump and Clinton, and the T-value and P-value.  
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** I see the same tests run as before, but with a higher t-value and a lower p value.
```{r}
