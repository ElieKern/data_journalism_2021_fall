...
**A1** [There are 33703 rows and 63 columns]
...
**A4** It seems that it is related to the fact Kanawha County is most populated and Wirt County is the least populated county in West Virginia.
...
**A5** GREENBRIAR HOTEL CORPORATION had the loan, for $8,912,202.  The business is a refined landmark resort set on 11,000 acres of wooded grounds, and it has retained 500 jobs. 
...
**A6** Jim Justice owns the company, and my source is propublica.org.  
...
**A7** What seems newsworthy is that Greenbrier Hotel Corporation has the highest loan amounts by far, and it happens to be owned by the state's governor.  This seems a little fishy, and could be worthy of further investigation. 
```{r}
west_virginia_ppp %>%
  group_by(name) %>%
  summarise(
  ...
  What would I need to do to store it as a variable object?  I am unsure of how that works.
```
...
**A8** Greenbrier county seems to not be a leader in total loans amount. This may show that the individual loan to greenbrier hotel corporation is an anomaly, further raising eyebrows as to the connection of Jim Justice to this whole thing.
```{r}
# Write the code to produce your answer here
```west_virginia_ppp %>%
  group_by(project_county_name) %>%
