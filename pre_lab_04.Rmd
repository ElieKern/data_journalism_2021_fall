...
**Answer**
The message says "Error: Can't join on `x$zip` x `y$zip` because of incompatible types. ℹ `x$zip` is of type <character>>. ℹ `y$zip` is of type <double>>."  I think it means that the two files can't combine because they are not under the same format.
...
**Answer** Same rows are still missing some data.  For example, some ethnicities still say Unknown/NotStated.
