...
```{r}
apps_per_state <- barber_shops_ppp %>% 
  group_by(state) %>% 
  summarise(
    all_apps=n()  
  ) 
...
undisbursed_apps_per_state <-  barber_shops_ppp %>% 
  group_by(state) %>% 
...
new_data <-
apps_per_state %>%
  inner_join(undisbursed_apps_per_state, by=c("state" = "state")) %>% 
  filter(all_apps > 2413)

write_csv(new_data, "new_data.csv")
```
**A1.**
https://datawrapper.dwcdn.net/2t72u/3/
Illinois had the most PPP applications for barber shops, with 11,080.  Illinois also had the most applications for undisbursed amounts, with 1,713.
...
**A2.**
https://datawrapper.dwcdn.net/sQuOv/1/
This chart is different from the first one because it focus on the ratio of undisbursed amounts to total loans.  Analyzing it percentage-wise gives a fuller picture to me. 
```{r}
new_data_different <- new_data %>% 
  mutate(percentage_undisbursed=undisbursed_apps/all_apps*100) 
write_csv(new_data_different, "new_data_different.csv")
```
...
all_counties <- all_counties %>% 
  rename(all_loans=total)
all_and_kabbage <- 
  kabbage_counties %>%
  inner_join(all_counties, by=c("project_county_name" = "project_county_name", "project_state" = "project_state")) %>% 
  mutate(percentage_kabbage=kabbage_loans/all_loans*100) 

all_plus_fips <-
  all_and_kabbage %>% 
  inner_join(county_fips, by=c("project_state" = "state", "project_county_name" = "match_county")) %>% 

  write_csv(all_plus_fips, "all_plus_fips.csv")
```
**A3.**  
https://datawrapper.dwcdn.net/My2X9/2/
Kabbage Loans are mostly found in the southeastern United States.  The state of California has almost none.  I noticed that Desoto County, Florida, had no data displayed. I have no strong idea as to why this is, so perhaps this because the county was intentionally withholding such information.  